    ...
    def forward(self, data_dict):
        ...
        for i in range(len(o5)):
            if i == 0:
                ...
                x6 = self.AvgPool2(x5[:o5[i]].unsqueeze(0))
                # o6[i] = i * 128 + 128
            else:
                p6 = torch.cat((p6, self.AvgPool1(p5[o5[i - 1]:o5[i]].unsqueeze(0))), 0)
                x6 = torch.cat((x6, self.AvgPool2(x5[o5[i - 1]:o5[i]].unsqueeze(0))), 0)
        # points
        # p6 = p6.view(len(o5), -1, 3)
        # x6 = x6.view(len(o5), -1, 512)
...
def pointtransformer2_feature_extractor(**kwargs):
    model = PointTransformerV2(**kwargs)
    return model