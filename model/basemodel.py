...
from model.pointtransformer2.pointtransformer2_semseg import offset2batch, batch2offset
import random
...
class CondNet(nn.Module):
    def __init__(self, config):
        ...
        self.scene_final_layer = nn.Linear(self.config.scene_group_size * self.scene_post_layer_out_size,
                                           self.config.scene_feat_size)
    ...
    def _process_scene_model_static_dict2(self, model_dict):
        static_dict = {}
    ...
    # def forward_ptv1(self, scene, lang, body, need_atten_score: bool = False):
    #     B, _ = body.shape
    #
    #     ## scene processing
    ...
    #
    #     co_attention_output, atten_score = self.co_attention_layer(raw_fall,
    #                                                                src_key_padding_mask=padding_mask_all)  # <B, NS+NL, C=512>
    ...
    def forward(self, scene, lang, body, need_atten_score: bool = False):
        B, _ = body.shape
        ## scene processing
        p6, x6 = self.scene_model(scene)
        # scene_xyz.detach(), scene_feat.detach() ## don't fintune
        # p6, x6, o6 = pxos
        # p5, x5, o5 = pxos[-1] # position <B * NS, 3>, feature <B * NS, 512>, offset <B>
                # o6[i] = i * 128 + 128
        _, NS, _ = p6.view(B, -1, 3).shape
        raw_fscene = x6.view(B, -1, 512)  # <B, NS, 512>
        ...
        ## post process
        ## scene, fuse all tokens
        scene_feature = torch.cat([scene_hidden_states, p6.view(B, -1, 3)], dim=-1)  # <B, NS, C=512 + 3>
        scene_feature = self.scene_process_layers(scene_feature)  # <B, NS, 16>
        scene_feature = self.scene_final_layer(scene_feature.view(B, -1))  # final_layer(<B, 16 * NS>) = <B, 512>
        ## language, use CLS token feature
        lang_feature = lang_hidden_states[:, 0, :]
        ...
        ## output
        if need_atten_score:
            return fusion_feat, regress_center, atten_score, p6.view(B, -1, 3)
...
class MotionEncoder(nn.Module):
    ...
    def forward(self, x, cond_z, seq_mask):
        ...
        h0 = torch.zeros(2, B, self.hidden_size // 2, dtype=torch.float32,
                         device=cond_z.device)  # <2, B, hidden_size>, use bidirectional rnn
        w_feats, hn = self.sequential_ecoder(packed_x, h0)  # _ , <2, B, hidden_size>
        w_feats, max_ = nn.utils.rnn.pad_packed_sequence(w_feats, batch_first=True,
                                                         total_length=nframe)  # <B, L, 2 * hidden_size>
...
class MotionDecoder(nn.Module):
    ...
    def forward(self, z_h, cond_z, tgt_mask):
        ...
        output = self.seqTransDecoder(tgt=timequeries, memory=rec_hidden,
                                      tgt_key_padding_mask=tgt_mask)  # <nframe, B, D>
...
class MotionModel(nn.Module):
    def __init__(self, config):
        ...
        self.encoder = MotionEncoder(self.config.input_size, self.config.hidden_size, self.config.condition_latent_size,
                                     self.config.z_size)
        self.decoder = MotionDecoder(self.config.input_size, self.config.hidden_size, self.config.condition_latent_size,
                                     self.config.z_size)
