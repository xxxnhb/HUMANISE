...
def test(args, dataset='val', eval_rec=True):
    ...
    if eval_rec:
        solver._report_rec_metric_results(write_file=os.path.join(args.log_dir, 'metric/recon.json'))
    else:
        solver._report_sample_metric_results(k=10, sample_type='fixed', write_file=os.path.join(args.log_dir, 'metric/sample.json'))
...
if __name__ == '__main__':
    ...
    test(args, eval_rec=True) # set eval_rec=False to compute sample metrics
